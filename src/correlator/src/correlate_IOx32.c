...
// Convert 64-bit to two 32-bit
typedef union uint64to32
{
        uint64_t u64;
        uint32_t u32[2];
}uint64to32;
                 ...
                 helper_t *helperData)
{
                            ...
                            &index) == 1)
    {
        ...
        tempVar=(uint64to32)strtoul(helperData->word,
                                    &strtoul_ptr,
                                    10);
        tableData->SNPtable[tableData->tableIndex*
                            tableData->compSize+
                            tableData->compIndex++]=tempVar.u32[0];
        tableData->SNPtable[tableData->tableIndex*
                            tableData->compSize+
                            tableData->compIndex++]=tempVar.u32[1];
        if(eol == 1)
            break;
    }
                     ...
                     int *counter)
{
                            ...
                            &index) == 1)
    {
        ...
        switch(elementIndex)
        {
            ...
            case 2: // ID
                ...
                {
                    fprintf(stderr,"ERROR: word not found at writeToTable_x32\n");
                    exit(1);
                }
                break;
    ...
    if(lineSkipped != 1)
    {
        ...
        if(!rnps_flag)
        {
            ...
            strcpy(tableData->IDtable[tableData->tableIndex],ID);
            binaryDeduction(statesNum, nodeData->snipSize, (helperData->snipChar));
            compressSnip_x32(tableData, helperData->snipChar, nodeData->snipSize);
            assert(tableData->BCtable[tableData->tableIndex] <=
                   (unsigned int)nodeData->snipSize);
                ...
                do
                {
                    ...
                    if(status == 1 && eol == 1)
                    {
                           ...
                           pos <= nodeData->posWmax)
                        {
                            if(threadData->mdf)
                            {
                                writeMDF_x32(tableData,
                                             helperData);
                            }
                            else
                            {
                                writeToTable_x32(nodeData,
                                                 tableData,
                                                 helperData,
                                                 pos,
                                                 &skippedLines,
