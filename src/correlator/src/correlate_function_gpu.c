...
void correlate_function_gpu(threadData_t *threadData)
{
    ...
    table_x32 *tableData=create_table_x32(tableSize1_s);
    ...
    while(1)
    {
        ...
        outFileType fpOut=FOPEN_OUT(new_node->output_file,"wb"); //wb1H best time/size
        assert(fpOut);
        setvbuf(fpOut, writeBuffer, _IOFBF, FILEBUFFERSIZE); //check here -MPAMPIS-
        ...
        compSize=(validData->valid_count/(sizeof(inputDataType_x32)*8)+
                (validData->valid_count%(sizeof(inputDataType_x32)*8)!=0?1:0));
        compSize2=(validData2->valid_count/(sizeof(inputDataType_x32)*8)+
                (validData2->valid_count%(sizeof(inputDataType_x32)*8)!=0?1:0));

        if(first_run)
        {
            first_run=0;
            // Reallocate memory of SNPtables according to the compressed size
            tableData->SNPtable=(inputDataType_x32*)realloc_buff(
                                    (void *)tableData->SNPtable,
                                    tableSize1_s*compSize,
                                    "inputDataType_x32");
            assert(tableData->SNPtable);
            tableData2->SNPtable=(inputDataType_x32*)realloc_buff(
    ...
    free_table_x32(tableData);
    free_table_x32(tableData2);
