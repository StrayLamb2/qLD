...
void compressSnip_x32(table_x32 *tableData,
                     ...
                     int size_snp)                      // snipSize,
{
    ...
    for(i=0; i < (unsigned int)size_snp; i++)
    {
        ...
        if(bitcount == (sizeof(inputDataType_x32)*8))
        {
    ...
    remainder=sizeof(inputDataType_x32)*8-bitcount;

    if(remainder < sizeof(inputDataType_x32)*8)
        temp<<=remainder;
    tableData->SNPtable[compIndex]=temp;
}
